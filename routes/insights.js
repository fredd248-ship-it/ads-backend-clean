  ...
  try {
    ...
    const distribution =
      totalEvaluations
        ? {
            strong: Math.round(
              (strong / totalEvaluations) * 100
            ),

            average: Math.round(
              (average / totalEvaluations) * 100
            ),

            weak: Math.round(
              (weak / totalEvaluations) * 100
            )
          }
        : null;
    ...
    const evaluationRate =
      totalDecisions
        ? Math.round(
            (evaluatedDecisions / totalDecisions) * 100
          )
        : 0;
    ...
    const weakest =
      categoryAverages.length
        ? categoryAverages[
            categoryAverages.length - 1
          ].category
        : null;

    // PATTERN DETECTION
    let primaryPattern =
      "Your decision-making is moderately consistent.";
    ...
    if (distribution) {
      ...
      if (distribution.weak >= 50) {
        ...
        primaryPattern =
          "A large portion of your decisions are underperforming.";

        recommendedFocus =
          "Slow down decisions and evaluate alternatives.";

      } else if (
        distribution.strong >= 60
      ) {

        primaryPattern =
          "You consistently make strong decisions.";

        recommendedFocus =
          "Maintain your current structured approach.";

      } else {

        primaryPattern =
          "Your results show inconsistency.";

        recommendedFocus =
          "Focus on reducing reactive decisions.";
      }
    }
    ...
    if (
      ...
      bestNextHabit =
        "Pause when decisions feel emotionally charged";
    }
